import runningIcon from '../../img/icon-run.svg';
import cyclingIcon from '../../img/icon-cycle.svg';
import otherIcon from '../../img/icon-other.svg';
import swimmingIcon from '../../img/icon-swim.svg';
import walkingIcon from '../../img/icon-walk.svg';

// TODO rework and optimize (no need for this script)
...
if (!m_helper) {
  var m_helper = {
    ...
    icon: function (iconType) {
      ...
      iconEl.setAttribute('name', iconType);
    ...
    image: function (link) {
      if (!link) {
        ...
        throwAwayDiv.style.display = 'none';
        return throwAwayDiv;
      }
      var imgEl = document.createElement('img');
      imgEl.classList.add('icon-training');
      imgEl.src = link;
      return imgEl;
    },
...
Multiselect.prototype = {
  ...
  _createInputField: function () {
    var input = m_helper.textField({
      ...
      dropdownIcon = m_helper.icon('chevron-down-outline');
    result = m_helper.div({ class: 'multiselect-input-div' });
    label.style.visibility = 'hidden';
    label.innerHTML = 0;
    result.appendChild(input);
    result.appendChild(dropdownIcon);
  ...
  _createItemList: function () {
    ...
    var selectAll = this._createItem('span', -1, 'All');
    // Delete icons at All
    var iconsAll = selectAll.querySelectorAll('.icon');
  ...
  _createItem: function (wrapper, value, text, selected) {
    var checkBox = m_helper.checkbox({
      ...
      label = m_helper.label();
    iconCheck = m_helper.icon('checkmark-outline');
    iconDot = m_helper.icon('ellipse');
    iconSport = m_helper.image(icons.get(value));
    textBox.className = 'multiselect-text';
    textBox.innerHTML = text;

    label.appendChild(iconSport);
    label.appendChild(textBox);
    label.appendChild(checkBox);
    label.appendChild(iconCheck);
    label.appendChild(iconDot);
    result.appendChild(label);
    return result;
  },
      ...
      .addEventListener('focus', function (event) {
        ...
        m_helper.each(window.multiselects, function (e) {
          if (
            ...
            // Stop getting duplicate calls when closing window
            // e._hideList(self);
          }
        });
      });
  ...
  _hideList: function (context, event) {
    ...
    activeItems.forEach((item) => {
      if (item.classList.contains('active'))
        selected.push(item.querySelector('input').dataset.val);
    });

    // Function that ouputs the selected fields on close
    // handleFilterMenu(selected);
...
// Hide on input field click
document
  .querySelector('.multiselect-input')
  .addEventListener('click', function (e) {
    const multiselectId = e.target
      .closest('.multiselect-wrapper')
      .id.split('_')[0];
    const targetMultiselect = document.multiselect(`#${multiselectId}`);

    if (targetMultiselect.state === 1) {
      targetMultiselect._hideList(targetMultiselect);
      targetMultiselect.state = 0;
    } else {
      targetMultiselect.state = 1;
      return;
    }
  });
